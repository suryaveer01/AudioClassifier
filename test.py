...
def test(model, train_dataloader, test_dataloader, loss_fn, device, epochs):
    ...
    for i in range(epochs):
        print(f"Epoch {i+1}")
        accuracy, average_loss =test_single_epoch(model, test_dataloader, loss_fn, device,epoch=i+1)
...
if __name__ == "__main__":
    ...
    resnet_model.load_state_dict(torch.load('resnet34_20_MEL_SPECTROGRAM.pth'))
    resnet_model.eval()
